...
export class ContentExtractorService {
  ...
  /**
   * Replace internal links to source post with links to translation
   * Example: href="https://example.com/?p=123" -> href="https://example.com/?p=456"
   */
  static replaceInternalLinks(
    content: string,
    sourcePostId: number,
    translationPostId: number,
    sourcePostLink?: string
  ): string {
    if (!content || !sourcePostId || !translationPostId) {
      return content;
    }

    const sourceIdStr = sourcePostId.toString();
    let result = content;

    // Pattern 1: href=".../?p=SOURCE_ID"
    result = result.replace(
      new RegExp(`href="([^"]*[?&])p=${sourceIdStr}([^"]*)`, 'g'),
      `href="$1p=${translationPostId}$2`
    );

    // Pattern 2: href=".../SOURCE_ID/"
    result = result.replace(
      new RegExp(`href="([^"]*/)${sourceIdStr}(/[^"]*)?"`, 'g'),
      `href="$1${translationPostId}$2`
    );

    // Pattern 3: If we have source post link, extract domain and replace full URLs
    if (sourcePostLink) {
      try {
        const sourceUrl = new URL(sourcePostLink);
        const domain = sourceUrl.origin;
        
        // Replace full URLs to source post with placeholder pattern
        result = result.replace(
          new RegExp(`href="${domain}/[^"]*[?/]${sourceIdStr}([^"]*)"`, 'g'),
          `href="${domain}/?p=${translationPostId}$1`
        );
      } catch (e) {
        // Invalid URL, skip
      }
